...
EOL;
    ...
    public function itShouldPassOrFailWith($success, PyStringNode $text): void
    {
        ...
        if (!$isCorrect) {
            $hasError = true;
            $outputMessage[] = 'Expected previous command to ' . strtoupper((string) $success) . ' but got exit code ' . $this->getExitCode();
        } else {
            $outputMessage[] = 'Command did ' . strtoupper((string) $success) . ' as expected.';
        }
    ...
    public function itShouldPassOrFail($success): void
    {
        ...
        throw new UnexpectedValueException(
            'Expected previous command to ' . strtoupper((string) $success) . ' but got exit code ' . $this->getExitCode()
        );
    }
